...
describe('req.setUrl()', () => {
    ...
    describe('url`/with/${"params"}`', () => {
        ...
        it('params are typesafe', () => {
            ...
            Req.create(HttpMethod.Delete, { ace: $.string })
                .setUrl`/orphans/${'ace'}`

            // @ts-expect-error 'bar' not in object
            Req.create(HttpMethod.Delete, { foo: $.string })
                .setUrl`/orphans/${'bar'}`

            // @ts-expect-error string/number's no objects or bools
            Req.create(HttpMethod.Post, { base: $.boolean })
                .setUrl`/orphans/${'base'}`

            Req.create(HttpMethod.Get, $({ case: $.number.optional }))
        ...
        // <{ id?: string, age?: number, query?: { name?: string, size?: string } }>
        const req = Req
            .create(HttpMethod.Get, { 
                id: $.string.optional, 
                age: $.number.optional, 
                query: $({ name: $.string.optional, size: $.string.optional }).optional
            })
            .setQueryKey('query')
        ...
        it('2 url param', () => {
            expect(
                req.fromData({ id: 'shirts', age: 34, query: nil })
            ).toEqual({
                method: HttpMethod.Get,
                body: undefined,
        ...
        it('1 url & query param', () => {
            expect(
                req.fromData({ id: 'shirts', age: nil, query: { name: 'joe' } })
            ).toEqual({
                method: HttpMethod.Get,
                body: undefined,
        ...
        it('2 query param', () => {
            expect(
                req.fromData({ id: nil, age: nil, query: { name: 'hey', size: 'large' } })
            ).toEqual({
                method: HttpMethod.Get,
                body: undefined,
    ...
    describe('url with pather function', () => {
        it('allows custom pathing', () => {
            const req = Req
                .create(HttpMethod.Get, { id: $.string, query: $.record($.or($.string, $.boolean, $.number)).optional })
                .setQueryKey('query')
                .setUrl(
                    data => {
                        const { id, ...rest } = data
            ...
            expect(req.fromData({ id: 'monkey ', query: nil })).toEqual({
                method: HttpMethod.Get,
                url: '/users/monkey'
            })
...
describe('req.linkHeaders()', () => {
    ...
    const request = Req.create<{ accessToken: string }>(HttpMethod.Post, { accessToken: $.string })
        .setUrl`/authenticate`
        .setHeaderLink(
            (headers, data) => {
    ...
    it('headers are not created with invalid values', () => {
        ...
        const { body, headers } = request.fromData({ accessToken: '' })
...
describe('req.match()', () => {
    ...
    const getTodo = Req.create(HttpMethod.Get, { id: $.string.optional })
        .setUrl`/todos/${'id'}`
