...
/*
    This is straight up ripped off from 
        https://github.com/feathersjs/feathers/blob/dove/packages/schema/src/hooks/resolve.ts 
  ...
  <T, H extends HookContext>(...resolvers: Resolver<T, H>[]) =>
      async (context: H, next?: NextFunction) => {
          ...
          const result = Array.isArray(data)
              ? await Promise.all(data.map((current) => 
                  runResolvers(resolvers, current, ctx, status))
              )
...
export const resolveAll = <H extends HookContext>(map: ResolveAllSettings<H>) => {
    ...
    middleware.push(resolveDispatch(map.dispatch as any))
