import { Typeable, TypeAssertion, TypeGuard, TypeOf, TypesOf } from './types'
import { Flags, F, DefaultFlags, HasFlag, SetFlag, IsFlag } from './flags'

import { Compile, Intersect } from '@benzed/util/lib'
...
const $ = null as unknown as $<DefaultFlags, []>
...
const isVector_ = $.shape({ x: $.number, y: $.number }).is

const isVector: TypeGuard<Vector> = $.shape({ x: $.number, y: $.number }).is
if (isVector(value))
    void value
