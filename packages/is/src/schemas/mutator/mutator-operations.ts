...
import { AnyValidate } from '../../../../schema/src/validator'
import { AnyMutator, Mutator, MutatorProperties, MutatorType as M } from './mutator'
...
export function * eachMutator(target: AnyValidate): Generator<AnyMutator> {
    while (isMutator(target)) {
        yield target
...
export function applyMutator<V extends AnyValidate, T extends M>(validate: V, type: T): ApplyMutator<V,T> {
    const applied = hasMutator(validate, type) 
        ? validate : 
        addMutator(validate, type)

    return applied as ApplyMutator<V,T>
}
...
export function removeMutator<V extends AnyValidate, T extends M>(
    ...
    const types = stack.map(mutator => mutator.mutator)
