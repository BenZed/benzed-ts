import { copy, Stateful, Structural } from '@benzed/immutable'
...
class Data<V> extends Traits.use(Node) { 
    ...
    get [Stateful.key](): Pick<this, 'data'> {
        return pick(this, 'data')
    }

    set [Stateful.key](state: Pick<this, 'data'>) {
...
describe('state', () => {
    ...
    it('get', () => {
        const state = Structural.getIn(list)
...
describe('typesafe builder', () => { 
    ...
    it('add', () => {
        ...
        expect(list2[Structural.key]).toEqual([new Data(5), new Data('ace'), new Data(true)])
    ...
    it('insert', () => {
        ...
        expect(list2[Structural.key]).toEqual([new Data(5), new Data(4), new Data('ace')])
        expectTypeOf(list2).toEqualTypeOf<NodeList<[Data<number>, Data<4>, Data<string>]>>()
    })

    it('swap', () => {
        const list2 = list.swap(0, 1)
        expect(list2[Structural.key]).toEqual([new Data('ace'), new Data(5)])
        expectTypeOf(list2).toEqualTypeOf<NodeList<[Data<string>, Data<number>]>>()
    })  

    it('remove', () => {
        expect(list.remove(0)[Structural.key]).toEqual([new Data('ace')])
        expect(list.remove(1)[Structural.key]).toEqual([new Data(5)])
        expectTypeOf(list.remove(0)).toEqualTypeOf<NodeList<[Data<string>]>>()
    })

    it('apply static', () => {
        const list2 = list.apply(0, new Data(10 as const))
        expect(list2[Structural.key]).toEqual(
            [{ data: 10 }, { data: 'ace' }]
        )
    ...
    it('apply', () => {
        const list2 = list.apply(0, 'data', 15)
        expect(list2[Structural.key]).toEqual(
            [{ data: 15 }, { data: 'ace' }]
        )
    })
