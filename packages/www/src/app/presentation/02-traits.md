...
class Vector2 extends Trait.use(Copyable, Comparable) {
    ...
    static readonly is: (input: unknown) => input is Vector2 = 
        isIntersection(
            Copyable.is,
            Comparable.is,
            isShape({
...
class VectorGameData extends GameData {
    ...
    get [Stateful.state](): Pick<this, 'x' | 'y'> {
...
class Multiply extends Trait.use(Callable<(i: number) => number>) {
...
```ts
type Multiply<T extends Damage> = T & { readonly by: number }
